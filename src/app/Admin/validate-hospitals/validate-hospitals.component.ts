...
export class ValidateHospitalsComponent implements OnInit {
  ...
  async openAccept(id: number) {
    (await this.dashbordService.ValidateHospital(id)).subscribe(data => {
  ...
  async Load() {
    this.isloading = true;
    (await this.accountService.GetValidateHospitalsAccount(this.PageNumber, this.pageSize)).subscribe(data => {
